...
- VARYNX-2.0 (this repo): https://github.com/M4urk/VARYNX-2.0
- varynx-prime: https://github.com/M4urk/varynx-prime
- varynx-prime-hub: https://github.com/M4urk/varynx-prime-hub
- varynx-prime-meta (post-rename target from varynx-prime-ecosystem): https://github.com/M4urk/varynx-prime-meta

Related but outside the Prime hierarchy:

- VARYNX_Android: https://github.com/M4urk/VARYNX_Android
...
| Windows Desktop | Controller | 5-tab Control Center (Overview, Devices, Activity, Mesh, Settings) with per-node detail views |
...
│  │ Modules  │  │ Behavior │  │ Responses│  │ Learning │        │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘        │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                     MESH ENGINE                           │   │
│  │  X25519/Ed25519 · AES-256-GCM · Trust Graph · Vector Clock│  │
│  │  Heartbeat · Threat Relay · Quorum · Delta Compression    │   │
│  └──────────────────────────────────────────────────────────┘   │
│                            │                                     │
├────────────────────────────┼─────────────────────────────────────┤
│  PLATFORMS                 │                                     │
│  ┌───────┐ ┌───────┐ ┌────┴──┐ ┌───────┐ ┌───────┐ ┌───────┐  │
│  │Android│ │Desktop│ │ Linux │ │WearOS │ │HomeHub│ │Pocket │  │
│  │Guard. │ │Contrl │ │ Node  │ │ Hub   │ │  IoT  │ │ Node  │  │
│  └───────┘ └───────┘ └───────┘ └───────┘ └───────┘ └───────┘  │
│                                            ┌───────┐            │
│                                            │Satellt│            │
│                                            │ Node  │            │
...
- **Kotlin Multiplatform** — Shared core across all 7 platforms
- **78 registered modules** — 47 active in 2.0, role-aware activation per platform
...
474 unit tests across 40 test suites, all passing.
...
| core (ReflexChainStress) | 5 | Multi-domain reflex chain stress |
| core (ConcurrencyStress) | 16 | Multi-threaded race condition tests |
| core (FullGuardianOrganismStress) | 11 | Full organism stress under load |
| core (HighVolumeEventStress) | 5 | High-throughput event pipeline |
| core (ScoringEngineStress) | 6 | Scoring engine under pressure |
| core (ThreatAnalytics) | 7 | Threat scoring and analytics |
...
| core (MeshPairing) | 2 | End-to-end pairing flow |
...
| core (HeadlessDaemonSmoke) | 11 | Daemon startup and lifecycle |
| core (IdentityModule) | 13 | Device identity persistence |
| core (DeviceRoleRegistry) | 11 | Role definitions and capabilities |
| core (ModuleActivationMatrix) | 3 | Role-aware module scoping |
...
| app (BatteryOptimization) | 9 | Android battery optimization |
...
- Desktop Control Center with 5 top-level tabs and per-node detail views
- Trust graph with non-transitive, user-approved trust
- 474 unit tests across 40 test suites
...
Public exposure controls are enforced by policy and automation:

- Private core/engine/security implementation remains in private repositories.
- Public repository content is limited to public-safe assets and documentation.
- Automated public safety scanning runs in CI before merges.

See `tools/security/PUBLIC_REPO_SPLIT_POLICY.md` for split rules and `tools/security/public_safety_scan.py` for the enforcement scanner.
